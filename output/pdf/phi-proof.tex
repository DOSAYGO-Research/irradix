...
\large A stronger characterization, a proof, and exact algorithms}
...
for packing. Finite-precision implementations are distinguished from
...
\emph{even shift} language \cite{glasscock}. The equivalence between
...
comparison. If many lengths repeat, block widths or compressed length
runs are further opportunities.

\paragraph{The prime-density claim needs a different baseline.}
Interpreting codewords as binary gives a sparse, biased integer set.
Among $n=1,\ldots,1000$ the repository's 94 mapped primes are reproducible,
but only 382 mapped values are odd. Asymptotically the odd proportion is
$1-1/\ph=\ph^{-2}$: each quotient contributes one zero-ending child,
and any second child ends in one. Prime density scales roughly with
inverse logarithmic magnitude, not inverse magnitude; dividing 16.8\%
by eight is unjustified. A parity-adjusted logarithmic heuristic gives
about 107 primes for this sample, rather than 21. This is a heuristic,
not a prime-distribution theorem; other residue biases remain.
